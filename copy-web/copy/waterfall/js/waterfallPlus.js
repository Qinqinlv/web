...
// function littleWaterfall(parent) {

//     // 这个函数中只试着排列每次ajax请求后的6张图片
//     // 调用该函数的时候，img标签已经被加载到了页面中，
//     // 接着就同步请求图片数据
//     // 此时我们就从父盒子中，取出这六张图片进行重新排列
...
//     // 获取父盒子中最后6张图片
//     var imgs = [].slice.call(items,len-6);



//     for (var i = 0; i < imgs.length; i++) { //遍历数组items的每个块框元素
...
//             imgs[i].style.left = imgs[minHIndex].offsetLeft + 'px';

//             // 将当前图片的高度追加到minIndex的位置
//             pinHArr[minHIndex] += imgs[i].offsetHeight;
